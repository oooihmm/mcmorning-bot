...
@bot.tree.command(name="기상게시글", description="기상 인증 게시글을 생성합니다.")
async def create_wake_post(interaction: discord.Interaction):
    ...
    if channel is None:
        await interaction.response.send_message(
            "❌ 기상인증 채널을 찾을 수 없습니다.", ephemeral=True
        )
        return

    thread = await channel.create_thread(
        name="☀️ 오늘의 기상 인증", content="8:00까지 이 게시글에 답글을 달아주세요!"
    )
    ...
    await interaction.response.send_message(
        f"☀️ 기상 인증 게시글을 생성했습니다!\n{thread.thread.jump_url}", ephemeral=True
    )
